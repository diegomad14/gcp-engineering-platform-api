...
def test_provider_keeps_public_repositories_on_github_and_fail_opens_billing(monkeypatch):
    service = _service()
    private = mock.Mock(return_value=False)
    usage = mock.Mock()
    monkeypatch.setattr(orchestrator.github_release_control, "repository_is_private", private)
...
def test_provider_opens_persistent_circuit_only_for_confirmed_quota(monkeypatch):
    service = _service()
    opened = mock.Mock()
    monkeypatch.setattr(orchestrator.github_release_control, "repository_is_private", lambda _: True)
    ...
    monkeypatch.setattr(orchestrator.github_actions_quota, "current_usage", lambda: None)
    assert orchestrator._provider(service) == "github_actions"
    ...
    assert orchestrator._provider(service) == "cloud_build"
    assert opened.call_args.kwargs["reason"] == "persistent_github_actions_billing_circuit"
...
def test_open_circuit_persists_then_propagates_to_private_repositories(monkeypatch):
    ...
    monkeypatch.setattr(
        orchestrator.catalog, "get_services",
        lambda: SimpleNamespace(services=[_service(), SimpleNamespace(repository="owner/public")]),
    )
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(orchestrator.github_release_control, "set_repository_execution_mode", set_mode)
...
def test_pr_redelivery_resumes_and_cancels_only_superseded_cloud_build(monkeypatch):
    ...
    ignored = _execution(execution_id="other", provider="github_actions", pull_request_number=7)
    save = mock.Mock()
    cancel = mock.Mock()
    start = mock.Mock()
    ...
    monkeypatch.setattr(orchestrator, "_submit_if_managed", lambda execution, _: execution)
    payload = {
        "action": "synchronize",
        "number": 7,
    ...
    assert orchestrator.handle_pull_request(payload, delivery_id="delivery") == [current]
...
def test_main_push_requires_fast_forward_and_reuses_reserved_execution(monkeypatch):
    ...
    monkeypatch.setattr(orchestrator.github_release_control, "is_fast_forward", fast_forward)
    payload = {
        "ref": "refs/heads/main", "after": HEAD, "before": BASE,
        "repository": {"full_name": REPOSITORY},
    }
    ...
    payload["before"] = "0" * 40
    with pytest.raises(orchestrator.ReleaseOrchestratorError, match="differential base"):
        orchestrator.handle_push(payload, delivery_id="delivery")
...
def test_health_probe_closes_circuit_only_after_real_started_success(monkeypatch):
    ...
    circuit = {
        "state": "open",
        "probe": {
            "status": "requested", "repository": REPOSITORY,
            "workflow": workflow, "nonce": "bound-nonce",
        },
    }
    recorded = mock.Mock()
    closed = mock.Mock()
    ...
    monkeypatch.setattr(orchestrator.executor_circuits, "close_after_successful_probe", closed)
    monkeypatch.setattr(orchestrator.github_release_control, "workflow_run", lambda *args: run)
    monkeypatch.setattr(orchestrator.github_release_control, "set_repository_execution_mode", set_mode)
    monkeypatch.setattr(
        orchestrator.catalog,
        "get_services",
        lambda: SimpleNamespace(services=[_service(), SimpleNamespace(repository="owner/other")]),
    )
    payload = {
        "path": f"owner/repo/{workflow}", "event": "workflow_dispatch",
        "display_title": "eng-platform-health-bound-nonce", "id": 42,
    }
...
def test_unbound_health_probe_cannot_close_circuit(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _: {
            ...
            "probe": {
                "status": "requested", "repository": REPOSITORY,
                "workflow": workflow, "nonce": "expected",
            },
        },
    )
    run = mock.Mock()
    monkeypatch.setattr(orchestrator.github_release_control, "workflow_run", run)
    assert orchestrator._close_circuit_from_probe(REPOSITORY, {"path": "other.yml"}) is False
    assert orchestrator._close_circuit_from_probe(
        REPOSITORY,
        {"path": workflow, "event": "workflow_dispatch", "display_title": "eng-platform-health-forged"},
    ) is True
...
def test_probe_variable_update_failure_preserves_open_circuit(monkeypatch):
    ...
    monkeypatch.setattr(orchestrator.executor_circuits, "record_probe", lambda *args, **kwargs: None)
    closed = mock.Mock()
    monkeypatch.setattr(orchestrator.executor_circuits, "close_after_successful_probe", closed)
    monkeypatch.setattr(
        orchestrator.catalog,
    ...
    def set_mode(repository, mode):
        ...
        if mode == "github_actions" and sum(
            recorded_mode == "github_actions" for _, recorded_mode in calls
        ) == 2:
    ...
    payload = {
        ...
        "display_title": "eng-platform-health-expected", "id": 43,
    }
    assert orchestrator._close_circuit_from_probe(REPOSITORY, payload) is True
